  ...
  function handlePreviousClick() {
    setModalType((prevType) => {
      if (prevType) {
        ...
        const type =
          index - 1 < 0 ? "phosphate retention" : modalTypesArray[index - 1];
        return type ?? null;
  ...
  function handleNextClick() {
    setModalType((prevType) => {
      if (prevType) {
        const index = modalTypesArray.indexOf(prevType);
        const type =
          index + 1 === modalTypesArray.length
            ? "clays"
            : modalTypesArray[index + 1];
        return type ?? null;
      } else return prevType;
    });
  }
  ...
  return (
    <>
      <div ref={sectionRef} className={styles["section"]}>
        ...
        >
          ph determines the amount and type of charge on soil minerals and
          organic matter (the exchange complex), as well as the amount of
          protons (H+) or hydroxyls (OH-) in the soil solution; driving overall
