...
export const ExchangeCapacitySection = () => {
  ...
  useEffect(() => {
    ...
    const svgs: [string, typeof modalType][] = [
      ["#exchange_capacity_clay_svg__Layer_2", "anion exchange"],
    ];
      ...
      <div ref={sectionRef} className={styles["section"]}>
        ...
        >
          ...
          <Text type="p" weight="thin" size="md" className="sm:w-1/2">
            som also has large surface area and charge (varies with pH),
            contributing significantly to a soil's water and nutrient-holding
            capacity.
          </Text>
        </div>
      </div>
      {modalType && (
        <GenericModal {...modalTypeMap[modalType]} onClose={handleCloseModal} />
      )}
    </>
  );
};
