...
export const PoreSpaceSection = (props: {
  assignRef: (el: null | HTMLDivElement) => void;
}) => {
  const isMobile = useMedia("(max-width: 680px)", false);
  ...
  useEffect(() => {
    ...
    const interactivePopupSvgs = popupSvgs.map(([id, popupId]) => {
      const popupSvg = sectionContainer.querySelector(
        popupId
      ) as SVGGElement | null;
    ...
    <>
      ...
      >
        ...
        <Text type="p" weight="thin" size="md" className={styles["p-60"]}>
          the open spaces within and between soil aggregates. the more pore
          space, the less dense the soil, and the easier for roots to grow and
          air/water to flow.
        </Text>
