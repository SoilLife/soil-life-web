    ...
    <div ref={props.assignRef} className={styles["section"]}>
      ...
      <div className="flex flex-col gap-4 h-full w-full items-center sm:flex-row sm:gap-8">
        <Text
          ...
          weight="light"
          size="md"
          className={`text-center sm:w-1/2 ${styles["p-70"]}`}
        >
          the distinct layers that develop are called{" "}
          <Text type="span" weight="bold" size="md">
            horizons
          </Text>
          , and the way they are arranged is called a soil profile. like
          chapters in a book, soil horizons provide a{" "}
          <Text type="span" weight="bold" size="md">
        ...
        <div ref={svgContainerRef} className="relative w-full h-full sm:w-1/2">
          ...
          {popups["organic"] && (
            <Popup {...popupMap["organic"]} className="sm:left-[70%] top-0" />
          )}
          {popups["topsoil"] && (
            <Popup
              {...popupMap["topsoil"]}
              className="sm:left-[70%] top-[5%]"
            />
          )}
          {popups["eluviated zone"] && (
            <Popup
              {...popupMap["eluviated zone"]}
              className="sm:left-[70%] top-[20%]"
            />
          )}
          {popups["accumulation zone"] && (
            <Popup
              {...popupMap["accumulation zone"]}
              className="sm:left-[70%] top-[30%]"
            />
          )}
          {popups["parent material"] && (
            <Popup
              {...popupMap["parent material"]}
              className="sm:left-[70%] top-[60%]"
            />
          )}
          {popups["bedrock"] && (
            <Popup
              {...popupMap["bedrock"]}
              className="sm:left-[70%] bottom-0"
            />
          )}
        </div>
      </div>
...
function Popup({
  ...
  return (
    <div
      className={`absolute bg-white shadow-lg p-4 max-w-[256px] w-full ${className}`}
    >
