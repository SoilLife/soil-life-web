    ...
    <>
        ...
        {Object.keys(media).map((key, index) => {
          ...
          if (media[key]) {
            let filteredMedia = media[key]
              ?.filter((medium) =>
                search
                  ? medium.Title?.toLowerCase()?.includes(search.toLowerCase())
                  : true
              )
              ?.filter((medium) => {
                let found = false;
      ...
      >
        ...
        modalMedia?.[mediaIndex]?.mediaType === "video" ? (
          ...
          />
        ) : modalMedia?.[mediaIndex]?.mediaType === "local-image" ? (
          <img
          ...
          <div
            className="overflow-y-auto max-w-full mx-auto max-h-[90%] "
            ref={imageKitContainerRef}
          >
            <Image
              fill
