...
const navLinks = [
  ...
  { name: "web of soil", slug: "/web-of-soil/food" },
  ...
  return navLinks.map(({ name, slug }) => {
    return (
      <li key={name} className="text-center cursor-pointer">
        <Link href={slug} tabIndex={isOpen ? undefined : -1}>
          <Text
            type="h3"
            weight="light"
            size="lg"
  ...
  return (
    <>
     ...
     `}
      ...
      {isMenuOpen && (
        <div
          className={`bg-gray-900 bg-opacity-25 absolute top-0 left-0 h-full w-full z-10`}
        />
      )}
    </>
  );
