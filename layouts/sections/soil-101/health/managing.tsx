...
export const ManagingSection = (props: {
  assignRef: (el: null | HTMLDivElement) => void;
}) => {
  const [activeHeader, setActiveHeader] =
    useState<keyof typeof headerSvgMap>("no-tillage");
  ...
  useEffect(() => {
    ...
    const sectionContainer = sectionRef.current?.querySelector(
      "#managing-soil-svg-container"
    ) as HTMLDivElement;
