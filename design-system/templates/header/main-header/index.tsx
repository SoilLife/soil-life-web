...
const navLinks = [
  ...
  },
  {
    ...
    slug: "/web-of-soil",
    submenu: [
      { name: "food", slug: "?section=food", icon: "food.svg" },
      { name: "fiber", slug: "?section=fiber", icon: "fiber.svg" },
      { name: "filter", slug: "?section=filter", icon: "filter.svg" },
      {
        name: "foundations",
        slug: "?section=foundations",
        icon: "foundations.svg",
      },
      {
        name: "farmaceuticals",
        slug: "?section=farmaceuticals",
        icon: "farmaceuticals.svg",
      },
      { name: "fun", slug: "?section=fun", icon: "fun.svg" },
    ],
  },
  { name: "media", slug: "/media" },
  {
    ...
    submenu: [
      ...
      {
        name: "with your legislators",
        slug: "/legislation",
        icon: "legislators.svg",
      },
...
function createNavLinks() {
  return navLinks.map(({ name, slug, submenu }) => {
    return (
      <li
        key={name}
        className="relative group text-center cursor-pointer xl:text-center xl:p-2 xl:py-0"
      >
        ...
        {submenu && (
          <ul
            className="hidden w-max opacity-0 absolute left-1/2 bg-white text-left shadow-2xl
            transform -translate-y-full -translate-x-1/2 transition-all duration-100 ease-in
            group-hover:translate-y-0 group-hover:opacity-100 group-hover:block
            group-focus-within:block group-focus-within:translate-y-0 group-focus-within:opacity-100"
          >
            {submenu?.map((item) => (
                ...
                <Link href={slug + item.slug}>
                  <a>
                    <div className="flex items-center space-x-8">
                      ...
                      />
                      <h3 className="font-acre-regular text-[24px]">
                        {item.name}
                      </h3>
                    </div>
                  </a>
    ...
    <>
      ...
      >
        <nav className="container relative flex items-center justify-center xl:justify-between">
          ...
          <ul className="hidden w-full xl:flex xl:justify-center xl:space-x-8 2xl:space-x-16">
            {createNavLinks()}
          </ul>
