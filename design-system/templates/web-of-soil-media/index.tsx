...
export const WebOfSoilMedia = memo(function ({ filter }: { filter: string }) {
  ...
  function handleModalOpen({ key, index }: { key: keyof typeof filteredMedia; index: number }) {
    return () => {
      setMediaIndex(index);
                  ...
                  {media?.map((medium, index) => {
                    return (
                      <MediaContainer
                        ...
                        onClick={handleModalOpen({ key: mediaTitle as keyof typeof filteredMedia, index })}
                        href={medium.URL}
