...
const videos = [
  "https://www.youtube.com/watch?v=uIW-aYHE60k",
  "https://www.youtube.com/watch?v=Nf3VgIiymKw",
  "https://www.youtube.com/watch?v=7g67sI-kAPs",
  "https://www.youtube.com/watch?v=VLv67zkZWgM",
  "https://www.youtube.com/watch?v=fXsvGdt27Y0",
  "https://www.youtube.com/watch?v=_xer83m6vzU",
];
...
export default function WebOfSoilPage() {
  ...
  const { graph: foundationsGraph } = useWebOfSoils(
    "foundations data structure"
  );
  const { graph: farmaceuticalGraph } = useWebOfSoils(
  ...
  useEffect(() => {
    ...
    const fpSlidesNav = Array.from(
      document.querySelectorAll(".fp-slidesNav")
    ) as HTMLDivElement[];
    if (fpNav) {
    ...
    return () => {
      for (const section of sections) {
        const anchors = Array.from(
          section.querySelectorAll(".fp-slidesNav a")
  ...
  function handleGraphEvent(
    type: "food" | "filter" | "fiber" | "fun" | "farmaceutical" | "foundation"
  ) {
    return {
      select({ nodes, edges }: { nodes: string[]; edges: string[] }) {
        ...
        const graphs: {
          [Type in typeof type]: { nodes: Node[]; edges: Edge[] };
        } = {
          ...
          if (edges.length === 1) {
            ...
            if (edge) {
              ...
              function findEdges(edgeId: string) {
                const connection = graphs[type].edges.find(
                  (e) => e.id === edgeId
                );
                const allEdges = graphs[type].edges.filter(
                ...
                if (connection && allEdges.length === 1) {
                  ...
                  if (node) {
                    ...
                    const foundEdge = graphs[type].edges.find((edge) =>
            ...
            const selectedNode = graphs[type].nodes.find(
              (node) => node.id === id
            );
                ...
                <Section>
                  ...
                  <Slide className="relative">
                    <Image
                      ...
                      url="/6Fs/Fiber_Slide_NkVYdxIN7-t.jpg"
                    />
                    <div
                      className={`p-6 absolute top-0 left-0 h-full w-full flex flex-col items-center justify-center
                  ...
                  <Slide className="relative">
                    <Image
                      className="object-cover"
                      url="/6Fs/Foundation_sl5IYmaDB.jpg"
                    />
                    <div
                   ...
                   lg:p-0 lg:h-auto lg:left-auto lg:top-auto lg:flex-row lg:bottom-[20%]`}
                    >
                      ...
                      <div className="lg:w-[760px]">
                        ...
                        >
                          from paints and pottery to playing fields and parks,
                          soils provide the grounds for music, sports, and art!
                        </Text>
                        <div className="mt-[15%] flex justify-center sm:mt-10">
                          <button
                ...
                <Section>
                  ...
                  <Slide>
                    <VisGraph
                      graph={filterGraph}
                      options={options}
                      events={handleGraphEvent("filter")}
                    />
                    <SlideButtons
                  ...
                  <Slide>
                    <VisGraph
                      graph={foundationsGraph}
                      options={options}
                      events={handleGraphEvent("foundation")}
                    />
                    <SlideButtons
                  ...
                  <Slide>
                    <VisGraph
                      graph={farmaceuticalGraph}
                      options={options}
                      events={handleGraphEvent("farmaceutical")}
                    />
                    <SlideButtons
          ...
          <div className="mt-10 mx-auto flex-grow">
            {nodeSelections.map((node) => {
              if (node.active && !node.to.length) {
                ...
                const edges = graphs[activeHeader]?.edges.filter(
                ...
                if ((edges?.length ?? 0) > 1) {
                  ...
                  function findConnectedNodes(node: Node) {
                    const connectedNodes =
                      graphs?.[activeHeader]?.nodes.filter((n) =>
                        n.to.includes(node.id)
                      ) ?? [];
